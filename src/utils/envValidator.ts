...
export const validateEnv = (): boolean => {
  ...
  // 누락된 환경 변수가 있으면 콘솔에 경고 메시지 출력 (오류가 아닌 경고로 변경)
  if (missingVars.length > 0) {
    console.warn('필수 환경 변수가 설정되지 않았습니다:', missingVars.join(', '));
    console.warn('일부 기능이 제한될 수 있습니다. .env 파일을 확인해주세요.');
    // return false; // 앱이 실행되지 않는 것을 방지
  }
